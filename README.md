# 🚀 自动生图 + 写博客 + 发布飞书：AI 测评博主的一站式神器 (gongji-comfyui-feishu-kit)

**这个项目的核心价值，是为你打造一条极速的“AI 测评与内容生产流水线”。**

无论你是：
- 🌟 **模型测评师**：出了新的开源/闭源 AI 生图模型，想要快速跑批测试，并一键生成测评博客发布。
- 💡 **提示词创作者**：看到一个绝佳的提示词创意（Prompt），想要快速拓展跑图（比如批量生成不同角色/风格），并迅速整理成图文并茂的文章发布。

过去你可能需要在生图工具、本地文件夹、编辑器和发布平台之间来回手动存图、复制粘贴。现在，只需给出一个想法或一套提示词模板，它就能帮你：
...
⚠️ **必读：飞书自建应用权限设置**
为了确保工具能够顺利创建飞书文档并上传生成的图片，你需要前往[飞书开放平台](https://open.feishu.cn/)的“开发配置 - 权限管理”页面，为你的自建应用搜索并开通以下权限，**开通后记得点击“创建版本”并发布**：
- `docx:document` (查看、评论、创建和更新文档)
- `drive:drive` (查看、创建、更新和删除云文档)
- `docs:document.media:upload` (上传新版文档附件及图片，**这一条最容易漏，如果不申请，图片将无法显示！**)
